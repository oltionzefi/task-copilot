---
title: "Task Copilot MCP Server"
description: "Configure the Task Copilot MCP server"
---

Task Copilot exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Task Copilot itself.

<Note>
This page covers connecting **external MCP clients** to Task Copilot's MCP server. For configuring MCP servers **within** Task Copilot for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Task Copilot's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Task Copilot MCP server to any [supported coding agent](/supported-coding-agents) **within** Task Copilot.

1. In Task Copilot Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Task Copilot card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add Task Copilot MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "task_copilot": {
      "command": "npx",
      "args": ["-y", "task-copilot@latest", "--mcp"]
    }
  }
}
```
...
The Task Copilot MCP server provides the following tools for managing projects, tasks, and task execution:
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your Task Copilot project automatically.
...
### Internal Coding Agents (Within Task Copilot)

A powerful workflow involves using coding agents within Task Copilot that are also connected to the Task Copilot MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the Task Copilot interface
...
Raycast is a popular MCP client that can connect to Task Copilot's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Task Copilot server" />
</Frame>

Configure the Task Copilot MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Task Copilot MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
