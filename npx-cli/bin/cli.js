...
async function main() {
      ...
      .then((latest) => {
        if (latest && latest !== CLI_VERSION) {
          setTimeout(() => {
            ...
            console.log(`Run: npx task-copilot@latest`);
          }, 2000);
        }
      })
      .catch(() => {});
  ...
  if (isMcpMode) {
    await extractAndRun("task-copilot-mcp", (bin) => {
    ...
    await extractAndRun("task-copilot-review", (bin) => {
    ...
    console.log(`Starting task-copilot v${CLI_VERSION}${modeLabel}...`);
    await extractAndRun("task-copilot", (bin) => {
      if (platform === "win32") {
